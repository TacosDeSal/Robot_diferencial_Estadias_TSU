...
const int CH_LPWM    =     1;
const int CH_ENA     =     1;
const int PWM_MIN    =    500;  //490 a 12v,
...
void IRAM_ATTR P_control() {
    ...
    //float error = pos_setpoint - filtered_velocity;
    error = pos_setpoint - c;
    ...
    //float output = (KP * error) + (KD * derivative_term);
    output = (KP * error); 
    ...
    if(pos_setpoint == distance && abs(error)<1){
        ledcWrite(CH_LPWM, 0);
        ledcWrite(CH_RPWM, 0); 
...
void setup() {
    ...
    timerAttachInterrupt(timer_1, &P_control, true);
...
void loop() {
    ...
    if (trayectoria_activa) {
        ...
        KP = map(analogRead(PIN_POT_1), 0, 4095, 0, 2400) / 30.0f;
        KI = map(analogRead(PIN_POT_2), 0, 4095, 0, 400) / 2.0f;
        KD = map(analogRead(PIN_POT_3), 0, 4095, 0, 10000)/ 1000.0f;
