...
void IRAM_ATTR PID_control() {
    ...
    float error = dinamic_setpoint - filtered_velocity;
...
void loop() {
    ...
    if (trayectoria_activa) {
        ...
        KI = map(analogRead(PIN_POT_2), 0, 4095, 0, 200) / 10.0f;
        KD = map(analogRead(PIN_POT_3), 0, 4095, 0, 200) / 10.0f;
    ...
    if (ahora - ultimo_serial >= 10) {
        ...
        //Serial.print("I:");
        //Serial.print(integral_acc);
        //Serial.print(" error:");
        //Serial.print(dbg_error);
        //Serial.print(" out:");
        //Serial.print(dbg_output);
        //Serial.print(" pwm:");
        //Serial.println(dbg_pwm);
        ...
        //Serial.printf("SET:%.2f | vel:%.2f | raw:%.2f | err:%.2f | I:%.4f | out:%.2f | pwm:%d\n",
            //dbg_setpoint, dbg_velocity, dbg_raw_velocity,
            //dbg_error, dbg_integral, dbg_output, dbg_pwm);
    }
}