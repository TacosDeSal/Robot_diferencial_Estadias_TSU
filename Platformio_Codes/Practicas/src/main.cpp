...
// ── TB6612 Motor 1 ────          // ── TB6612 Motor 2 ────
const int PIN_ENB  = 15;           const int PIN_ENA  = 25;
const int PIN_IN3  = 4;            const int PIN_IN1  = 33;
const int PIN_IN4  = 5;            const int PIN_IN2  = 32;
...
// ── PWM ───────────────────────────────────────────────────────────────────────────────
const int PWM_FREQ       = 15000;               const int PWM_MIN_m1 =  225;
const int PWM_RESOLUTION =    12;               const int PWM_MAX_m1 =  4095;
const int CH_ENB = 0; 
const int CH_ENA = 1;                           const int PWM_MIN_m2 =  225;  
                                                const int PWM_MAX_m2 = 4095;  


//-Trapezoidal parameters Motor 1 ──────────        //-Trapezoidal parameters Motor 2 ────────
const float cruiser_vel_m1     =  5000.0f;          const float cruiser_vel_m2     =  5000.0f;  
const float max_acelaration_m1 =  5000.0f;          const float max_acelaration_m2 =  5000.0f;  
float distance_m1              =  15000.0f;         float distance_m2              =  20000.0f;  
...
volatile float peak_vel_m1 = 0.0f;                  volatile float escale_factor = 1.0f;


// ── PID parameters ──────────────────────────
float KP_m1 = 64.0f, KD_m1 = 2.33f;
float KP_m2 = 100.0f, KD_m2 = 18.0f;
...
// ════════════════════════════════════════════════════════════
void trapezoidal_times_m1(float escale_factor, float vel, float acel) {
...
// ════════════════════════════════════════════════════════════
void trapezoidal_times_m2(float escale_factor, float vel, float acel) {
...
// ════════════════════════════════════════════════════════════
void IRAM_ATTR trapezoidal_profile_m1(float escale_factor, float cruiser_vel_m1, float max_acelaration_m1) {
    ...
    if (distance_m1 < distance_m2){
        escale_factor = distance_m1 / distance_m2;
        cruiser_vel_m1 = escale_factor * cruiser_vel_m1;
        max_acelaration_m1 = escale_factor * max_acelaration_m1; 
    }
...
// ════════════════════════════════════════════════════════════
void IRAM_ATTR trapezoidal_profile_m2(float escale_factor, float cruiser_vel_m2, float max_acelaration_m2) {
    ...
    if (distance_m2 < distance_m1){
        escale_factor = distance_m2 / distance_m1;
        cruiser_vel_m2 = escale_factor * cruiser_vel_m2;
        max_acelaration_m2 = escale_factor * max_acelaration_m2; 
    }
...
void IRAM_ATTR motor1_routine() {
    trapezoidal_profile_m1(escale_factor, cruiser_vel_m1, max_acelaration_m1);
    PID_control_m1();
}

void IRAM_ATTR motor2_routine() {
    trapezoidal_profile_m2(escale_factor, cruiser_vel_m2, max_acelaration_m2);
    PID_control_m2();
}
...
void setup() {
    ...
    ledcSetup(CH_ENB, PWM_FREQ, PWM_RESOLUTION); ledcAttachPin(PIN_ENB, CH_ENB); ledcWrite(CH_ENB, 0);
    ...
    trapezoidal_times_m1(escale_factor, cruiser_vel_m1, max_acelaration_m1);
    trapezoidal_times_m2(escale_factor, cruiser_vel_m2, max_acelaration_m2);
}
...
void loop() {
    ...
    if (trayectoria_activa) {
        ...
        KD_m2 = map(analogRead(PIN_POT_2), 0, 4095, 0, 200) / 10.0f;
        //distance_m1 = map(analogRead(PIN_POT_3), 0, 4095, 0, 30000) / 1.0f;
    ...
    if (ahora - ultimo_serial >= 10) {
        ...
        Serial.print(">distancia motor 1:");      Serial.println(distance_m1);
        //Serial.print(">output_2:");      Serial.println(output_m2);
